    ...
    public class GetSetInBeforeEachExampleSpec : Spec<GetSetInBeforeEachExampleSpec.ClassWithDependencies>
    {
        ...
        public void Run()
        {
            ...
            Given("dependencies were set up in BeforeEachExample").Verify(() =>
                Then("it should get a value from the first dependency that was set up", () => result.ShouldBe(123)).
                Then("it should get a value from the second dependency that was set up", () => SUT.UseAnotherDependency().ShouldBe(789)));

            Given("dependency overridden in a Given", () =>
            {
                Get<IDependency>().GetValue().Returns(456); // TODO: if I understand the documentation correctly, NSubstitute always replaces the previous Stub
                Set<IAnotherDependency>(new AnotherDependency(987));
            }).Verify(() =>
                Then("it should get the new value from the first dependency", () => result.ShouldBe(456)).
                Then("it should get the new value from the second dependency", () => SUT.UseAnotherDependency().ShouldBe(987)));
        }
