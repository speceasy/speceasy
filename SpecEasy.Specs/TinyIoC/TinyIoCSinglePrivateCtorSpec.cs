    ...
    internal class TinyIoCSinglePrivateCtorSpec : Spec<SinglePrivateCtor>
    {
        public void ConstructorSelection()
        {
            ...
            Given("the type being constructed has a single private constructor").Verify(() =>
                Then("an exception should be thrown because we will not use private ctors to build the SUT instance", () => AssertWasThrown<Exception>(ex => ex.GetType().FullName.Should().StartWith("TinyIoC.TinyIoCResolutionException"))));
        }
    }
}